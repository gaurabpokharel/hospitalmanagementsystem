    ...
    <body style="color: black; background-color: #ecf5ff">
        ...
        <div class="container">
            ...
            <div class="side-bar" id="side-bar" style="color:white;background-color: #207dff">
                ...
                <div class="side-bar-item active" onclick="window.open('receptionistappointmentlist.jsp','_top')">
                     (   <%                    try {
                        Class.forName("com.mysql.jdbc.Driver").newInstance();
                        Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/hospital", "root", "");
                        Statement st = con.createStatement();
            ...
            <div class="body-content">
                <div class="top-header">Appointment Request List</div>

                <%
                    List<appointmentbean> list = appointmentDao.getAppointmentAllRecords();
                    request.setAttribute("list", list);
                %>  

                <table style="text-align: center;color:#0e4ca3">  
                    <tr><th>Id</th><th>Patient Name</th><th>Preferred date</th><th>Preferred time</th>  
                        <th>Email</th><th>Phone Number</th><th>Department</th> <th>Doctor</th>
                        <th>Allocate Doctor</th><th>Delete</th>
                            <c:forEach items="${list}" var="u"> 
                        <tr><td>${u.getId()}</td><td>${u.getPname()}</td> 
                            <td>${u.getApdate()}</td><td>${u.getAtime()}</td>
                            <td>${u.getEmail()}</td><td>${u.getPnumber()}</td> 
                            <td>${u.getDepartment()}</td><td>${u.getDoctor()}</td>
                            <td><a href="receptionistconfirmappointment.jsp?id=${u.getId()}"style="color:#0e4ca3" >Allocate</a></td>
