<%@page import="com.info.dao.staffDao"%>
<%@page import="com.info.method.staffBean"%>
...
<html>  
    <head>  
        ...
        <title>View All Doctor List</title> </script>
...
<div style="float: right; margin-right: 20px">
        ...
        <h1 style="text-align: center">Staff List</h1><hr style="width:75%; height:4px;background-color: black">  
        ...
        <table border="1" width="90%" style="text-align: center;margin-left: 5%">  
            <tr><th>Id</th><th>User name</th><th>Password</th><th>Role</th> <th>Delete</th></tr>  
                    <c:forEach items="${list}" var="u">  
                <tr><td>${u.getId()}</td><td>${u.getUsername()}</td><td>${u.getPassword()}</td> 
                    <td>${u.getRole()}</td><td><a href="deleteuser.jsp?id=${u.getId()}">Delete</a></td>
                </tr> 
                </c:forEach>  
        </table> 
        <h2 style="text-align: right">   <%
                try {
                    ...
                    String strQuery = "SELECT COUNT(username) FROM stafflogin";
                    ResultSet rs = st.executeQuery(strQuery);
                    String Count = "";
                    while (rs.next()) {
                        Count = rs.getString(1);
                        out.println("Total Number of Staff :" + Count);
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                }
