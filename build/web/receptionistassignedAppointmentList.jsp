...
<html>
    <head>
        ...
        <title>Assigned Appointment List</title>
    ...
    <body style="color: black; background-color: #ecf5ff">
        ...
        <div class="container">
            ...
            <div class="side-bar" id="side-bar" style="color: white;background-color: #207dff">
                ...
                <div class="side-bar-item active"  onclick="window.open('receptionistassignedAppointmentList.jsp','_top')" >
                    (   <%
                    try {
                        Class.forName("com.mysql.jdbc.Driver").newInstance();
                        Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/hospital", "root", "");
            ...
            <div class="body-content">
                <div class="top-header">Assigned Appointment List</div>

                <%
                List<appointmentbean> list = appointmentDao.getAssignedAppointmentAllRecords();
                request.setAttribute("list", list);
            %> 
            
            <table border="1" width="90%" style="text-align:center;color:#0e4ca3">  
                <tr><th>Id</th><th>Patient Name</th><th>department</th> 
                    <th>Doctor ID</th><th> Doctor Name</th> <th>Allocate Date</th><th>Allocate time</th> 
                        <c:forEach items="${list}" var="u"> 
                    <tr><td>${u.getId()}</td><td>${u.getPname()}</td><td>${u.getDepartment()}</td>
                        <td>${u.getDoctorid()}</td> <td>${u.getDoctor()}</td><td>${u.getFdate()}</td><td>${u.getTime()}</td></tr>
                    </c:forEach>  
            </table>
