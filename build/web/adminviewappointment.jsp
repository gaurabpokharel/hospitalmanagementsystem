...
<div style="float: right; margin-right: 20px">
        <form action="staffLogout">
            ...
            <div class="container">
             ...
             <h2 style="text-align: center">Visited Patient List</h2>  
          <hr style=" height:4px;background-color: black">

            <%
                List<appointmentbean> list = appointmentDao.getAllAdminRecords();

                request.setAttribute("list", list);
            %> 

            <table border="1" width="90%" style="text-align: center;margin-left: 30px">  
                <tr><th>Id</th><th>Patient Name</th><th>Appointment title</th><th>Appointment date</th>  
                    <th>Email</th><th>Phone Number</th><th>Department</th><th>Allocate Doctor</th><th>Amount</th><th>Delete</th>
                        <c:forEach items="${list}" var="u">
                    <tr><td>${u.getId()}</td><td>${u.getPname()}</td><td>${u.getAptitle()}</td>  
                        <td>${u.getFdate()}</td><td>${u.getEmail()}</td><td>${u.getPnumber()}</td> 
                        <td>${u.getDepartment()}</td><td>${u.getDoctor()}</td>
                        <td>${u.getAmount()}</td><td><a href="admindeleteappointment.jsp?id=${u.getId()}">Delete</a></td></tr>
                    </c:forEach>
            </table>   
            <h2 style="text-align: right">   <%
                try {
                    ...
                    String strQuery = "SELECT SUM(amount) FROM appointment";
                    ResultSet rs = st.executeQuery(strQuery);
                    String Count = "";
                    while (rs.next()) {
                        Count = rs.getString(1);
                        out.println("Total Amount :" + Count);
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                }
