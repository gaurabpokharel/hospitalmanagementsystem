        ...
        <div class="container">
            ...
            <div class="side-bar" id="side-bar" style="color: white;background-color: #207dff">
                ...
                %>)
                    ...
                    <div class="side-bar-item active" onclick="window.open('receptionistassignedBookingList.jsp','_top')">
                    (   <%
                    try {
                        Class.forName("com.mysql.jdbc.Driver").newInstance();
                        Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/hospital", "root", "");
            ...
            <div class="body-content">
                <div class="top-header">Confirm Booking List</div>

           
    

<%
                List<appointmentbean> list = appointmentDao.getAssignedBookingAllRecords();
            ...
            <table border="1" width="90%" style="text-align: center;color:#0e4ca3">  
                <tr><th>Id</th><th>Patient Name</th><th>Appointment title</th><th>Preferred date</th>  
                    <th>Symptoms</th><th>Suspected Disease</th><th> Doctor Name</th> <th>Allocate Date</th><th>Allocate time</th> 
                        <c:forEach items="${list}" var="u"> 
                    <tr><td>${u.getId()}</td><td>${u.getPname()}</td><td>${u.getAptitle()}</td>  
                        <td>${u.getApdate()}</td><td>${u.getSymptoms()}</td><td>${u.getDisease()}</td> 
                        <td>${u.getDoctor()}</td><td>${u.getFdate()}</td><td>${u.getTime()}</td></tr>
                    </c:forEach>  
            </table>  
